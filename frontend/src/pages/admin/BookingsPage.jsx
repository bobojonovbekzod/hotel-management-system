    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">Barcha check-in va check-outlar</p>
        </div>
      </div>
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-slate-400">Bron topilmadi</p>
          </div>
        ) : (
                ...
                {bookings.map((b) => (
                  <tr key={b.id} className="table-row cursor-pointer" onClick={() => setSelected(b)}>
                    <td className="table-td">
                      <div>
                        <p className="font-medium text-white">{b.primaryGuest?.firstName} {b.primaryGuest?.lastName}</p>
                        <p className="text-xs text-slate-500">{b.primaryGuest?.phone}</p>
                    ...
                    <td className="table-td">
                      <span className="font-bold text-white">#{b.room?.roomNumber}</span>
                    ...
                    <td className="table-td text-right">
                      <p className="font-bold text-white">{b.paidAmount?.toLocaleString()}</p>
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setSelected(null)}>
          <div className="modal-content p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-white">Bron #{selected.id}</h2>
              <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-white text-2xl">&times;</button>
            </div>
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-800/50 rounded-xl p-3">
                  <p className="text-slate-400 text-xs mb-1">Asosiy mehmon</p>
                  <p className="text-white font-semibold">{selected.primaryGuest?.firstName} {selected.primaryGuest?.lastName}</p>
                  <p className="text-slate-400">{selected.primaryGuest?.phone}</p>
                  <p className="text-slate-400">{selected.primaryGuest?.passportNumber}</p>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-3">
                  <p className="text-slate-400 text-xs mb-1">Xona</p>
                  <p className="text-white font-bold text-xl">#{selected.room?.roomNumber}</p>
                  <p className="text-slate-400">{selected.room?.roomType} | {selected.room?.floor}-qavat</p>
                </div>
              </div>
              {selected.additionalGuests?.length > 0 && (
                <div className="bg-slate-800/30 rounded-xl p-3">
                  <p className="text-slate-400 text-xs mb-2">Qo'shimcha mehmonlar:</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-800/50 rounded-xl p-3">
                  <p className="text-slate-400 text-xs">Kirish</p>
                  <p className="text-white">{format(new Date(selected.checkIn), 'dd.MM.yyyy HH:mm')}</p>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-3">
                  <p className="text-slate-400 text-xs">Chiqish</p>
                ...
                <div className="bg-slate-800/50 rounded-xl p-3">
                  <p className="text-slate-400 text-xs">To'langan</p>
                  <p className="text-emerald-400 font-bold">{selected.paidAmount?.toLocaleString()} so'm</p>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-3">
                  <p className="text-slate-400 text-xs">Jami narx</p>
                  <p className="text-white font-bold">{selected.totalPrice?.toLocaleString()} so'm</p>
                </div>
              </div>
              {selected.notes && (
                <div className="bg-slate-800/30 rounded-xl p-3">
                  <p className="text-slate-400 text-xs mb-1">Izoh:</p>
